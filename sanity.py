...
def sanity_check(test_filename, output_filename):
    ...
    # Just to check the kind of error that the model make
    # error_log_file = "parsing_errors.txt"  # Specify the error log file
    # # Replace apply with for loop and try-except
    # for index, row in output_df.iterrows():
    #     try:
    #         parse_string(row['prediction'])
    #         print('s')
    #     except Exception as e:
    #         error_message = f"Error parsing prediction at index {row['index']}: {e}\n"
    #         with open(error_log_file, "w") as log_file:  # Append errors to the log file
